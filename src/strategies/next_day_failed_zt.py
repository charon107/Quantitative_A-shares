...
def add_signal_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    3. close_below_high: 当日收盘价是否低于当日最高价
    """
    if df.empty:
        return df
    ...
    d["close_below_high"] = d["close"] < d["high"]
    return d
...
def get_signal_indices(df: pd.DataFrame) -> list:
    ...
    base_cond = (
        (d["high_breakout_pct"] > HIGH_BREAKOUT_MIN) &
        (d["close_below_high"]) &
        mask_year
    )
...
def compute_trades_buyclose_sellnextopen(df: pd.DataFrame) -> pd.DataFrame:
    """
    信号条件：
        1. 当日最高价相对昨日收盘价涨幅 > HIGH_BREAKOUT_MIN
        2. 当日收盘价 < 当日最高价
    ...
    for i in idxs:
        ...
        rows.append({
            ...
            "close_signal": float(d.at[i, "close"]),
