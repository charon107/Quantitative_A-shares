"""
Redis L2 持久化缓存 — 为 Streamlit 看板的 @st.cache_data 提供跨重启持久层。

设计：
  - L1: Streamlit @st.cache_data (进程内存, TTL 3600s)
  - L2: Redis (持久化, 经 parquet bytes 序列化)

Redis 不可用时优雅降级：所有操作静默跳过，不影响看板正常运行。
...
用法示例：
    ...
    def load_latest_day():
        ...
        result, _hit = try_load("load_latest_day", fallback_fn=compute)
        return result

缓存失效：
    # 数据刷新后清空所有缓存
    python -m src.data_collection.hf_sync clear-redis-cache
"""
import os
import json
import hashlib
...
def _get_redis():
    ...
    try:
        ...
        _redis_client = None
        logger.warning("Redis 不可用 (%s)，回退到仅 L1 缓存", exc)
    return _redis_client if _redis_available else None


def is_available() -> bool:
    """检查 Redis L2 缓存是否可用。用于数据状态页面展示。"""
...
def _serialize_df(df: pd.DataFrame) -> bytes:
    """DataFrame → parquet bytes (zstd 压缩)。"""
...
def _serialize(value: Any) -> tuple[bytes, str] | tuple[None, None]:
    """分发序列化。返回 (bytes, type_tag) 或 (None, None)。"""
...
def _retrieve(key: str) -> tuple[Any, dict | None]:
    """从 Redis 读取值。返回 (value, meta) 或 (None, None)。"""
    r = _get_redis()
    if r is None:
        return None, None
    try:
...
def invalidate_all() -> int:
    """
    清空当前版本的所有 Redis 缓存 key（SCAN + 批量删除）。
    返回删除的 key 数量。
    """
    r = _get_redis()
    if r is None:
